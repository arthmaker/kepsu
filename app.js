...
function validateBasics(){
  ...
  const phDesc  = $("phDesc").value;
  const phImg   = $("phImg").value;
  const phArticle = $("phArticle").value;
  if(!phTitle || !phLink || !phDesc || !phImg || !phArticle){
  ...
  if(!templateText.includes(phDesc))  missing.push(phDesc);
  ...
  const arrDesc  = lines($("descriptions").value);
  const arrImg   = lines($("images").value);
  const arrArticle = parseArticleBlocks($("articles").value);
  const n = Math.max(arrTitle.length, arrLink.length, arrDesc.length, arrImg.length);
  ...
  if(arrTitle.length !== n || arrLink.length !== n || arrDesc.length !== n || arrImg.length !== n){
    setStatus("bad", `ERROR: Jumlah baris tidak sama. Judul=${arrTitle.length}, Link=${arrLink.length}, Deskripsi=${arrDesc.length}, Gambar=${arrImg.length}`);
    report([{k:"Jumlah baris", v:`Judul=<code>${arrTitle.length}</code>, Link=<code>${arrLink.length}</code>, Deskripsi=<code>${arrDesc.length}</code>, Gambar=<code>${arrImg.length}</code>`}]);
    return { ok:false };
  }
  ...
  return { ok:true, phTitle, phLink, phDesc, phImg, phArticle, arrTitle, arrLink, arrDesc, arrImg, arrArticle, n };
}

function buildOutputs(payload){
  const { phTitle, phLink, phDesc, phImg, phArticle, arrTitle, arrLink, arrDesc, arrImg, arrArticle, n } = payload;
  const strict = $("chkStrict").checked;
  const nameFromLink = $("chkNameFromLink").checked;

  const templateMasked = maskTemplateWithTokens(templateText, phTitle, phLink, phDesc, phImg, phArticle);
  const outputs = [];
  const integrityErrors = [];
  const nameSet = new Set();

  for(let i=0;i<n;i++){
    const title = arrTitle[i], link = arrLink[i], desc = arrDesc[i], img = arrImg[i], article = arrArticle[i];

    let out = templateText
      .split(phTitle).join(title)
      .split(phLink).join(link)
      .split(phDesc).join(desc)
      .split(phImg).join(img)
      .split(phArticle).join(article);

    if(strict){
      const outMasked = maskOutputToTokens(out, title, link, desc, img, article);
